...
function Versiones({ id }: { id: string }) {
  ...
  if (!versiones.data?.length) {
    return <p className="text-sm text-slate-500">Sin rectificaciones: esta es la primera versión.</p>
  }
  return (
    <ol className="space-y-4">
      {versiones.data.map((v: VersionRemision) => (
        <li key={v.version} className="rounded-md border border-slate-200 p-4 text-sm">
          ...
          {v.motivoRechazo && (
            <p className="mt-1 text-red-700">Motivo del rechazo: {v.motivoRechazo}</p>
          )}
...
function Auditoria({ id }: { id: string }) {
  ...
  if (!auditoria.data?.length) return <p className="text-sm text-slate-500">Sin registros.</p>
  return (
      ...
      {auditoria.data.map((e) => {
        ...
        return (
          <li key={e.id} className="rounded-md border border-slate-200 p-3 text-sm">
            ...
            {e.motivo && <p className="mt-1 text-slate-700">Motivo: {e.motivo}</p>}
            {cambios.length > 0 && (
              <ul className="mt-2 space-y-0.5 text-xs text-slate-600">
                {cambios.map((c) => (
                  <li key={c.clave}>
                    <span className="font-mono">{c.clave}</span>: {valor(c.antes)} →{' '}
                    <span className="font-medium text-slate-900">{valor(c.despues)}</span>
                  </li>
                ))}
              </ul>
            )}
...
export function HistorialRemision({ remisionId }: { remisionId: string }) {
  ...
  const clase = (activa: boolean) =>
    `border-b-2 px-3 py-2 text-sm ${activa ? 'border-marca font-medium text-marca' : 'border-transparent text-slate-600 hover:text-slate-900'}`
